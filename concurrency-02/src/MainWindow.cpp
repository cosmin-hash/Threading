...
MainWindow::MainWindow() {
    setWindowTitle("Threading Model Visualiser - multi-producer bounded queues");
...
void MainWindow::onStats(int produced, int dropped, int skipped, int displayed,
                         const QString& status) {
    m_stats->setText(
        QString("Produced: %1    Dropped: %2    Skipped: %3    Displayed: %4      [%5]")
            .arg(produced).arg(dropped).arg(skipped).arg(displayed).arg(status));
}
