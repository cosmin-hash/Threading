...
if [ -d "$BUILD/PipelineViz.app" ]; then
    exec open "$BUILD/PipelineViz.app"
elif [ -x "$BUILD/PipelineViz" ]; then
    exec "$BUILD/PipelineViz" "$@"
else
    echo "PipelineViz not built yet. Build it first:" >&2
