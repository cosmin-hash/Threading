...
MainWindow::MainWindow() {
    setWindowTitle("Threading Model Visualiser - staged pipeline + thread pool");
...
void MainWindow::onStats(int produced, int stalls, int consumed,
                         const QString& status) {
    m_stats->setText(
        QString("Produced: %1    Throttle stalls: %2    Consumed: %3      [%4]")
            .arg(produced).arg(stalls).arg(consumed).arg(status));
}
