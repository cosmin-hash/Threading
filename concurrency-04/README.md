...
[`work_stealing_pool_core.cpp`](src/work_stealing_pool_core.cpp) (the console skeleton this app
visualises).
...
installed per-OS (with `libgl1-mesa-dev` / `libxkbcommon-dev` on Linux), the app is compiled,
and on Linux it is launched headlessly (`QT_QPA_PLATFORM=offscreen`) to confirm it starts and
its event loop runs. Each successful run uploads the built binary
(`WorkStealingViz` / `.exe` / `.app`) as an artifact. Add a status badge once the repo is public:
