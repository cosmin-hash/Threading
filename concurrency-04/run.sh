#!/usr/bin/env bash
# Launch the Threading Model Visualiser (work-stealing scheduler) on Linux / macOS.
...
if [ -d "$BUILD/WorkStealingViz.app" ]; then
    exec open "$BUILD/WorkStealingViz.app"
elif [ -x "$BUILD/WorkStealingViz" ]; then
    exec "$BUILD/WorkStealingViz" "$@"
else
    echo "WorkStealingViz not built yet. Build it first:" >&2
